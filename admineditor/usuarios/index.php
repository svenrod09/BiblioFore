<?php
// Verificar credenciales de la sesión (Usuarios de tipo 2 [user_type == 2 no] pueden acceder a este apartado)
session_start();
if (!isset($_SESSION['logged_in']) || $_SESSION['logged_in'] !== true) { // Si no existen credenciales de la sesión
    ...
    // Si un usuario de tipo = 2 (Editor) intenta acceder aquí se le redirecciona a la página de inicio
    if ($_SESSION['user_type'] == 2) {
        $_SESSION['permission_alert'] = "No tiene permisos para acceder a este apartado."; // Envía un mensaje de alerta
        header("Location: ../adminPage.php"); // Redirección a la página de inicio
    }
}
?>
...
<head>
    ...
    <title>BiblioFore | Usuarios</title>
...
<body>
            ...
            <div class="col-auto col-md-3 col-xl-2 px-sm-2 px-0 theme-bg">
                <div class="sticky-top d-flex flex-column align-items-center align-items-sm-start px-3 pt-2 text-white min-vh-100">
                    ...
                    <a href="../usuarios/index.php" id="usuarios" class="d-flex align-items-center pb-4 text-decoration-none items-color">
                        <i class="fas fa-users fs-4 fa-fw activo items-color"></i>
                        <span class="ms-2 d-none d-sm-inline activo fw-bold">Usuarios</span></a>
                        ...
                        <table id="dataTable" class="table table-striped table-bordered table-hover">
                            ...
                            <tbody>
                                ...
                                try {
                                    ...
                                    $query = "SELECT u.id, u.usuario, u.password, u.tipousuarioid, tu.nombre 
                        FROM usuarios AS u INNER JOIN tipousuario tu ON u.tipousuarioid = tu.id";
                                ...
                                ?>
                                        <tr>
                                            ...
                                            <td class="text-center">
                                                ...
                                                <a href="#" data-toggle="tooltip" title="Editar" data-bs-toggle="modal" data-bs-target="#editarModal" data-iduser="<?php echo $row['id'] ?>" data-nameuser="<?php echo $row['usuario'] ?>" data-usertype="<?php echo $row['tipousuarioid'] ?>" class="link-dark open-edit tooltip-link"><i class="fas fa-edit fs-5 me-3"></i></a>
                                                <!-- Eliminar (al hacer clic se envían a los modal los parámetros necesarios) -->
                                                <a href="#" data-toggle="tooltip" title="Eliminar" data-bs-toggle="modal" data-bs-target="#eliminarModal" data-iduser="<?php echo $row['id'] ?>" data-nameuser="<?php echo $row['usuario'] ?>" data-usertype="<?php echo $row['tipousuarioid'] ?>" class="link-dark open-delete tooltip-link"><i class="fas fa-trash fs-5 me-3"></i></a>
                                            </td>
                                        </tr>
                                <?php
                                    }
                        ...
                        <div class="modal-dialog">
                            <div class="modal-content">
                                <div class="modal-header">
                                    ...
                                    <h5 class="modal-title fw-bold" id="nuevoModalLabel">Nuevo Usuario:</h5>
                                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                </div>
                                <div class="modal-body">
                                    <form id="nuevoRegistroForm">
                                        <div class="mb-3">
                                            <input type="text" class="form-control input-style" title="Ingrese el nombre de usuario." name="usuarioC" id="usuarioC" placeholder="Usuario">
                                        </div>
                                        <div class="mb-3">
                                            <input type="text" class="form-control input-style" title="Ingrese la contraseña (mínimo 8 caracteres)." name="passwordC" id="passwordC" placeholder="Contraseña">
                                        </div>
                            ...
                            <div class="modal-content">
                                ...
                                <div class="modal-body">
                                    <form id="editarRegistroForm">
                                        <div class="mb-3">
                                            <label class="fw-bold" for="usuarioE">Usuario:</label>
                                            <input type="text" class="form-control input-style" title="Ingrese el nombre de usuario." name="usuarioE" id="usuarioE" placeholder="Usuario" readonly>
                                        </div>
                                        <div class="mb-3">
                                            <label class="fw-bold" for="passwordE">Nueva Contraseña:</label>
                                            <input type="text" class="form-control input-style" title="Ingrese la contraseña (mínimo 8 caracteres)." name="passwordE" id="passwordE" placeholder="Contraseña">
                                        </div>
                                        <div class="justify-content-center text-center">
                                            <p id="errormsgE" class="d-none text-danger fw-bold errormsg">La contraseña debe ser mayor a 8 caracteres.</p>
                                        </div>
                                    </form>
                                </div>
                                <div class="modal-footer">
                                    <button type="button" class="btn btn-danger" data-bs-dismiss="modal"><i class="fas fa-times-circle me-2"></i></i>Cerrar</button>
                                    <button type="button" id="edit" class="btn btn-dark" disabled><i class="fas fa-save me-2"></i>Guardar</button>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="modal fade" id="eliminarModal" tabindex="-1" aria-hidden="true">
                        <div class="modal-dialog">
                            <div class="modal-content">
                                ...
                                <div class="modal-body">
                                    ¿Realmente deseas eliminar a <strong id="name-delete"></strong>?
                                </div>
                                <div class="modal-footer">
    ...
    <script>
        $(document).ready(function() {
            ...
            var inputUsuarioC = document.getElementById('usuarioC');
            var inputPasswordC = document.getElementById('passwordC');
            var inputUsuarioE = document.getElementById('usuarioE');
            var inputPasswordE = document.getElementById('passwordE');
            ...
            inputUsuarioC.addEventListener('input', function() { // Input de usuario
                // Validar que el usuario no esté vacío y que la contraseña sea de 8 caracteres
                if (inputUsuarioC.value == '' || inputPasswordC.value.length < 8) {
            ...
            inputPasswordC.addEventListener('input', function() { // Input de contraseña
                // Validar que el usuario no esté vacío y que la contraseña sea de 8 caracteres
                if (inputUsuarioC.value == '' || inputPasswordC.value.length < 8) {
                    // El input no es válido, mostrar un mensaje de error
                    msgErrorC.classList.remove("d-none");
            ...
            inputPasswordE.addEventListener('input', function() { // Input de contraseña
                // Validar que la nueva contraseña sea de 8 caracteres
                if (inputPasswordE.value.length < 8) {
                    // El input no es válido, mostrar un mensaje de error
            ...
            $("#submit").click(function() {
                ...
                // Validar si un usuario existe en la BD (No se puede ingresar un usuario existente)
                $.ajax({ // Petición AJAX
                    type: "POST", // Método POST
                    url: "validate_user.php", // Archivo que contiene la consulta de selección
                    data: { // Enviar datos
                        usuario: usuario
                    },
                    dataType: 'json',
                    success: function(response) {
                        if (response.success) { // Usuario existente
                            // Enviar alerta 
                            $("#nuevoModal").modal('hide'); // Ocultar Modal
                            $("#alertMessage").html("¡Error! Usuario ya existente :(").addClass("alert-danger").fadeIn();
                            resetearInput();
                            setTimeout(function() {
            ...
            $('.open-edit').click(function() {
                ...
                $("#usuarioE").val(nameUser);

                $('#edit').click(function() {
                    // Validar que no se edite un usuario administrador
                    if (userType == 1) {
                        // Enviar alerta 
                        $("#editarModal").modal('hide'); // Ocultar Modal
                        $("#alertMessage").html("¡Error! No se puede editar un usuario administrador :(").addClass("alert-danger").fadeIn();
                        resetearInput();
                        setTimeout(function() {
                            $("#alertMessage").fadeOut();
                        }, 3000);
            ...
            $('.open-delete').click(function() {
                ...
                var nameUser = $(this).data('nameuser');
                var userType = $(this).data('usertype');
                $("#name-delete").html(nameUser);

                // Confirmar eliminación
                $('#delete').click(function() {
                    // Validar que no se elimine un usuario administrador
                    if (userType == 1) {
                        // Enviar alerta 
                        $("#eliminarModal").modal('hide'); // Ocultar Modal
                        $("#alertMessage").html("¡Error! No se puede eliminar un usuario administrador :(").addClass("alert-danger").fadeIn();
                        setTimeout(function() {
                            $("#alertMessage").fadeOut();
                        }, 3000);
                    } else {
